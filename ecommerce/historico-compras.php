...
<html lang="pt-br">
  <head>
    ...
    <title>Histórico de Compras - ELentes</title>
  ...
  <body class="fade-down">
    ...
    <div class="container m-t-3">
      <div class="row">
        ...
        <!-- Account Sidebar -->
        <div class="col-sm-12 col-md-3 m-b-3">
          <div class="account-picture">
              <img src="./controls/controlUsuario.php?getImagem=<?php echo $_SESSION['usuario']; ?>" alt="" class="img-thumbnail">
          </div>
          
            <h4 class="text-center m-b-3">               
                <?php                  
                    foreach($usuario->getUsuario($_SESSION['usuario']) as $var) {
                        echo $var['nome'];                        
                    } 
          ...
          <ul class="nav nav-pills nav-stacked">
            <li role="presentation"><a href="perfil-usuario">Perfil</a></li>
            <li role="presentation" class="active"><a href="historico-compras">Histórico de Compras</a></li>
            <li role="presentation"><a href="alterar-senha-usuario">Alterar Senha</a></li>
          </ul>
        </div>
        <!-- End Account Sidebar -->
        ...
        <div class="col-sm-12 col-md-9">
          <div class="title m-b-2"><span>Histórico de Compras</span></div>
              <div class="panel-body table-scroll nopaddintop">
                    <table class="table table-bordered table-hover table-striped hd-center tr-hover alinharv">
                      <thead>
                        <tr>
                            <th>Pedido</th><th>Data</th><th>Valor</th><th>Detalhes</th><th>Refazer Compra</th>
                        </tr>
                      </thead>
                      <tbody>
                        ...
                        <?php 
                            ...
                            foreach ($pedido->funcaoGetHistorico($_SESSION['usuario']) as $var) {
                        
                        ?>
                          
                        <tr data-toggle="modal" data-target="#detalhesProduto">
                            <td><?php echo $var['idPedido']; ?></td>
                            <td><?php echo date_format(date_create($var['data']), 'd/m/Y'); ?></td>
                            <td>R$ <?php echo $var['precoTotal']; ?></td>
                            
                            <td class="col-sm-1 text-right">
                                <form method="POST" action="detalhes-pedido">
                                    <input type="text" class="hidden" name="idPedido" value="<?php echo $var['idPedido']; ?>"/>
                                    <button type="submit" class="btn btn-default btn-sm">Detalhes do Pedido  <i class="fa fa-list-ul"></i></button>
                                </form>
                            </td>
                            <td class="col-sm-1 text-right">
                                <form method="POST" action="./controls/controlPedido.php">
                                    <input type="text" class="hidden" name="idPedidoRefazerCompra" value="<?php echo $var['idPedido']; ?>"/>
                                    <button type="submit" class="btn btn-theme btn-sm">Refazer Compra <i class="fa fa-shopping-cart"></i></button>
                                </form>
                            </td>
                        </tr>

...
if (!isset($_POST['idProduto'])) {
    ...
    foreach ($produto->getProduto($id) as $var) {
        ...
        foreach ($produto->funcaoGetIntervaloEsfericoProduto($id) as $var2) {
            
            $intervaloEsferico[] = $var2['grauEsferico'];
        }
        
        $_SESSION['listaEsferico'] = $intervaloEsferico;
        
        // Lista Curva Base
        
        $intervaloCurvaBase = array();
        
        foreach ($produto->funcaoGetIntervaloCurvaBaseProduto($id) as $var2) {
            
            $intervaloCurvaBase[] = $var2['curvaBase'];
        }
        
        $_SESSION['listaCB'] = $intervaloCurvaBase;
        
        // Lista Diâmetro
        
        $intervaloDiametro = array();
        
        foreach ($produto->funcaoGetIntervaloDiametroProduto($id) as $var2) {
            
            $intervaloDiametro[] = $var2['diametro'];
        }
        
        $_SESSION['listaDiametro'] = $intervaloDiametro;
        ...
        if ($var['torica']) {
            ...
            foreach ($produto->funcaoGetIntervaloCilindroProduto($id) as $var2) {

                $intervaloCilindro[] = $var2['grauCilindro'];
            }

            $_SESSION['listaCilindro'] = $intervaloCilindro;
            
            // Lista Eixo

            $intervaloEixo = array();

            foreach ($produto->funcaoGetIntervaloEixoProduto($id) as $var2) {

                $intervaloEixo[] = $var2['eixo'];
            }

            $_SESSION['listaEixo'] = $intervaloEixo;
        }
        ...
        if ($var['multifocal']) {
            ...
            foreach ($produto->funcaoGetIntervaloAdicaoProduto($id) as $var2) {

                $intervaloAdicao[] = $var2['grauAdicao'];
            }

            $_SESSION['listaAdicao'] = $intervaloAdicao;            
        }
        ...
        if ($var['colorida']) {
            ...
            foreach ($produto->funcaoGetIntervaloCorProduto($id) as $var2) {

                $cores[] = $var2['cor'];
            }

            $_SESSION['listaCor'] = $cores;            
        }
    }
...
<html lang="pt-br">
  <head>
    ...
    <title>Alterar Produto - ELentes</title>
    <!-- Bootstrap -->
    ...
    $(function ($) {
        ...
        $(window).load(function() {
            ...
            $('#tabelaEsferico').load('./controls/controlProduto.php?inicioEsf='+0+'&fimEsf='+0+'&passosEsf='+0+' #tabelaEsferico');
            $('#tabelaCurvaBase').load('./controls/controlProduto.php?inicioCB='+0+'&fimCB='+0+'&passosCB='+0+' #tabelaCurvaBase');
            $('#tabelaDiametro').load('./controls/controlProduto.php?inicioDiametro='+0+'&fimDiametro='+0+'&passosDiametro='+0+' #tabelaDiametro');
            ...
            foreach ($produto->getProduto($id) as $var) {
                if ($var['torica']) {
                    ?>
                        ...
                        $('#tabelaCilindro').load('./controls/controlProduto.php?inicioCilindro='+0+'&fimCilindro='+0+'&passosCilindro='+0+' #tabelaCilindro');
                        $('#tabelaEixo').load('./controls/controlProduto.php?inicioEixo='+0+'&fimEixo='+0+'&passosEixo='+0+' #tabelaEixo');
                    <?php
                }
                ...
                if ($var['multifocal']) {
                    ?>
                        ...
                        $('#tabelaAdicao').load('./controls/controlProduto.php?inicioAdicao='+0+'&fimAdicao='+0+'&passosAdicao='+0+' #tabelaAdicao');
                    <?php
                }
                ...
                if ($var['colorida']) {
                    ...
                    ?>
                        ...
                        $('#tabelaCor').load('./controls/controlProduto.php?nomeCor #tabelaCor');
                    <?php
                }
            }
            ?>
        ...
        <div class="col-sm-12 m-b-3">
          ...
          <div class="row">
              <form id="formulario" name="formProduto" method="POST" action="./controls/controlProduto.php">
                ...
                <input type="text" class="form-control hidden" id="idProdutoEdit" name="idProdutoEdit" value="<?php echo $var['idProduto'] ?>" />
                ...
                <select class="form-control selectpicker" data-live-search="true" id="fornecedorInput" name="fornecedorInput">
                    <?php
                        ...
                        $sql = "SELECT idFornecedor, razaoSocial FROM fornecedores WHERE idFornecedor = ".$var['idFornecedor']."";
                        $consulta = DB::PDO($sql);
                        ...
                        foreach($consulta as $var2) {
                            echo "<option value='".$var2['idFornecedor']."'>".$var2['razaoSocial']."</option>";
                        }
                    
                  

                        $sql2 = "SELECT idFornecedor, razaoSocial FROM fornecedores WHERE idFornecedor != ".$var['idFornecedor']."";
                        $consulta2 = DB::PDO($sql2);

                        $consulta2->execute();
                        $consulta2 = $consulta2->fetchAll();

                        foreach($consulta2 as $var2) {
                            echo "<option value='".$var2['idFornecedor']."'>".$var2['razaoSocial']."</option>";
                        }
                ...
                <div class="form-group col-sm-6">
                    ...
                    <input type="text" class="form-control" id="nomeInput" name="nomeInputEdit" placeholder="Nome" value="<?php echo $var['nome'] ?>" />
                  </div>
                <div class="form-group col-sm-6">
                    <label for="valorInput">Valor</label>
                    <input type="text" class="form-control" id="valorInput" name="valorInput" value="<?php echo $var['valorUnit']; ?>" placeholder="Valor">
                </div>
                ...
                <div class="form-group col-sm-12">
                    <div class="form-group col-sm-12">
                        <div class="form-group col-sm-4">
                            <label><input type="radio" name="uso" id="checkDescartavel" <?php if ($var['descartavel']) { echo "checked='true'"; } ?> value="descartavel"><span> Descartável</span></label>
                        </div>
                        <div class="form-group col-sm-4">
                            <label><input type="radio" name="uso" id="checkAnual" <?php if ($var['anual']) { echo "checked='true'"; } ?> value="anual"><span> Anual</span></label>
                        </div>
                    </div>
                    <div class="form-group col-sm-12">
                        <div class="form-group col-sm-4">
                            <label id="mensal"><input type="radio" name="troca" id="checkMensal" <?php if ($var['mensal']) { echo "checked='true'"; } ?> value="mensal"><span> Mensal</span></label>
                        </div>
                        <div class="form-group col-sm-4">
                            <label id="diaria"><input type="radio" name="troca" id="checkDiaria" <?php if ($var['diaria']) { echo "checked='true'"; } ?> value="diaria"><span> Diária</span></label>
                        </div>
                        <div class="form-group col-sm-4">
                            <label id="quinzenal"><input type="radio" name="troca" id="checkQuinzenal" <?php if ($var['quinzenal']) { echo "checked='true'"; } ?> value="quinzenal"><span> Quinzenal</span></label>
                        </div>
                    </div>
                    <div class="form-group col-sm-12">
                        <div class="form-group col-sm-4">
                            <label><input type="radio" name="tipo" id="checkEsferica" <?php if ($var['esferica']) { echo "checked='true'"; } ?> value="esferica"><span> Esférica</span></label>
                        </div>
                        <div class="form-group col-sm-4">
                            <label><input type="radio" name="tipo" id="checkTorica" <?php if ($var['torica']) { echo "checked='true'"; } ?> value="torica"><span> Tórica</span></label>
                        </div>
                        <div class="form-group col-sm-4">
                            <label><input type="radio" name="tipo" id="checkMultifocal" <?php if ($var['multifocal']) { echo "checked='true'"; } ?> value="multifocal"><span> Multifocal</span></label>
                        </div>
                    </div>
                    <div class="form-group col-sm-12">
                        <div class="form-group col-sm-4">
                            <label><input type="checkbox" id="checkColorida" <?php if ($var['colorida']) { echo "checked='true'"; } ?> name="checkColorida"><span> Colorida</span></label>
                        </div>
                    </div>
                </div>
            ...
            <div id="tabelaEsferico"></div>
              ...
              <div id="olho" class="hidden">
                ...
                <div class="form-group col-sm-12 nomarginbotton">
                  <div class="form-group col-sm-12 nomarginbotton">
                      <div class="form-group col-sm-6">
                          <label><input name="olho" type="radio" id="radioOlhoDominante" <?php if ($var['multifocal'] && $var['olho'] == 1) { echo "checked='true'"; } ?> value="dominante"><span> Olho Dominante</span></label>
                      </div>
                        <div class="form-group col-sm-6">
                          <label><input name="olho" type="radio" id="radioOlhoNaoDominante" <?php if ($var['multifocal'] && $var['olho'] == 0) { echo "checked='true'"; } ?> value="ndominante"><span> Olho Não Dominante</span></label>
                      </div>
                  </div>
                </div>
              </div>
              ...
              <div class="form-group col-sm-12">
                ...
                <textarea style="resize: none" class="form-control" rows="5" id="descricaoInput" name="descricaoInput"><?php echo $var['descricao']; ?></textarea>
              </div>
              ...
              <hr style="width: 97.5%;" />
                ...
                <div class="form-group col-sm-6">
                    ...
                    <input type="text" class="form-control" id="marcaInput" name="marcaInput" value="<?php echo $var['marca']; ?>" placeholder="Marca">
                  </div>
                <div class="form-group col-sm-6">
                    <label for="tipoInput">Tipo</label>
                    <input type="text" class="form-control" id="tipoInput" name="tipoInput" value="<?php echo $var['tipo']; ?>" placeholder="Tipo">
                </div>
                <div class="form-group col-sm-6">
                    <label for="materialInput">Material</label>
                    <input type="text" class="form-control" id="materialInput" name="materialInput" value="<?php echo $var['material']; ?>" placeholder="Material">
                </div>
                <div class="form-group col-sm-6">
                    <label for="visitintInput">Visitint</label>
                    <input type="text" class="form-control" id="visitintInput" name="visitintInput" value="<?php echo $var['visitint']; ?>" placeholder="Visitint">
                </div>
                ...
                <div class="col-xs-12">
                    <button id="cadastrar" type="submit" class="btn btn-theme"><i class="fa fa-long-arrow-right"></i> &MediumSpace; Salvar</button>
                </div>
                <?php } ?>
             </form>
          </div>

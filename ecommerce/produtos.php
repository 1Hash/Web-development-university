...
require_once("conexao.class.php");
    ...
    $where = "";
        ...
        foreach($consulta as $var) {
            ...
            if (isset($_POST["fornecedor$cont"])) {
                
                $fornecedoresArray['id'][$indice] = $_POST["fornecedor$cont"];
                $fornecedoresArray['pos'][$indice] = $cont;
                
                $id = $_POST["fornecedor$cont"];
                
                $where .= "AND `idFornecedor` = $id ";
                $indice++;
            }
            else {
                $fornecedoresArray['id'][$indice] = -1;
                $fornecedoresArray['pos'][$indice] = -1;
                $indice++;
            }

            //echo "<div class='checkbox'><label><input type='checkbox' name='fornecedor$cont' onChange='this.form.submit()' value='".$var['idFornecedor']."'><span> ".$var['razaoSocial']."</span></label></div>";
            $cont++;
    ...
    if (!isset($_POST['busca'])) {
        if (isset($_POST['colorida'])) {
            $colorida = 1;
            $where .= "AND `colorida` = $colorida ";
        }

        if (isset($_POST['esferica'])) {
            $esferica = 1;
            $where .= "AND `esferica` = $esferica ";
        }

        if (isset($_POST['torica'])) {
            $torica = 1;
            $where .= "AND `torica` = $torica ";
        }

        if (isset($_POST['multifocal'])) {
            $multifocal = 1;
            $where .= "AND `multifocal` = $multifocal ";
        }

        if (isset($_POST['mensal'])) {
            $mensal = 1;
            $where .= "AND `mensal` = $mensal ";
        }

        if (isset($_POST['quinzenal'])) {
            $quinzenal = 1;
            $where .= "AND `quinzenal` = $quinzenal ";
        }

        if (isset($_POST['diaria'])) {
            $diaria = 1;
            $where .= "AND `diaria` = $diaria ";
        }

        if (isset($_POST['anual'])) {
            $anual = 1;
            $where .= "AND `anual` = $anual ";
        }

        if (isset($_POST['descartavel'])) {
            $descartavel = 1;
            $where .= "AND `descartavel` = $descartavel ";
        }
    }
      ...
      <div class="row">
        ...
        <div class="col-sm-3 hidden-xs">
          ...
          <div class="filter-sidebar">
            <div class="title"><span>Fornecedor</span></div>
            ...
            <?php
                ...
                foreach($consulta as $var) {
                    ...
                    $fornecedores = "";
                    $fornecedores .= "<div class='checkbox'><label><input type='checkbox' name='fornecedor$cont2'";
                    
                    if($cont2 == $fornecedoresArray['pos'][$cont2 - 1]) {
                        //echo "".$fornecedoresArray['pos'][$cont2 - 1]." ||".$cont2;
                        //if ($fornecedoresArray['id'][$cont2 - 1] == $var['idFornecedor']) {
                            $fornecedores .= " checked='checked' ";
                        //}
  
                    }
                    
                    $fornecedores .= "onChange='this.form.submit()' value='".$var['idFornecedor']."'><span> ".$var['razaoSocial']."</span></label></div>";
          ...
          <div class="filter-sidebar">
            <div class="title"><span>Tipo</span></div>
            <div class="checkbox"><label><input type="checkbox" <?php if ($anual) { echo "checked='checked'"; } ?> name="anual" onChange="this.form.submit()"><span> Anual</span></label></div>
            <div class="checkbox"><label><input type="checkbox" <?php if ($descartavel) { echo "checked='checked'"; } ?> name="descartavel" onChange="this.form.submit()"><span> Descartável</span></label></div>
            <div class="checkbox"><label><input type="checkbox" <?php if ($esferica) { echo "checked='checked'"; } ?> name="esferica" onChange="this.form.submit()"><span> Esférica (Miopia / Hipermetropia)</span></label></div>
            <div class="checkbox"><label><input type="checkbox" <?php if ($torica) { echo "checked='checked'"; } ?> name="torica" onChange="this.form.submit()"><span> Tórica (Astigmatismo)</span></label></div>
            <div class="checkbox"><label><input type="checkbox" <?php if ($multifocal) { echo "checked='checked'"; } ?> name="multifocal" onChange="this.form.submit()"><span> Multifocal (Presbiopia)</span></label></div>
            <div class="checkbox"><label><input type="checkbox" <?php if ($colorida) { echo "checked='checked'"; } ?> name="colorida" onChange="this.form.submit()"><span> Colorida</span></label></div>
          </div>
          <div class="filter-sidebar">
            <div class="title"><span>Troca</span></div>
            <div class="checkbox"><label><input type="checkbox" <?php if ($mensal) { echo "checked='checked'"; } ?> name="mensal" onChange="this.form.submit()"><span> Mensal</span></label></div>
            <div class="checkbox"><label><input type="checkbox" <?php if ($quinzenal) { echo "checked='checked'"; } ?> name="quinzenal" onChange="this.form.submit()"><span> Quinzenal</span></label></div>
            <div class="checkbox"><label><input type="checkbox" <?php if ($diaria) { echo "checked='checked'"; } ?> name="diaria" onChange="this.form.submit()"><span> Diária</span></label></div>
          </div>
          </form>
        </div>
        <!-- End Filter Sidebar -->
        ...
        <div class="col-sm-9">
          <div class="title"><span>Produtos</span></div>
            ...
            if (isset($_POST['busca'])) {
                ...
                $sqlProdutos = "SELECT * FROM produtos WHERE `status` IS NULL AND nome LIKE '%$busca%' LIMIT $numeroSelecao,$totalProdutosPorPagina";
            }
            else {           
                $sqlProdutos = "SELECT * FROM produtos WHERE `status` IS NULL $where LIMIT $numeroSelecao,$totalProdutosPorPagina";
            }
...
function paginacao($totalProdutos, $totalProdutosPorPagina, $offset) {
    ...
    if ($paginaAtual == 1) {
        ...
        $p = $paginaAtual - 1;
        $paginas .= "<li><a href='$urlSite/pagina-produtos/1'><span>&laquo;</span></a></li>";
        $paginas .= "<li><a href='$urlSite/pagina-produtos/$p'><span>&lsaquo;</span></a></li>";
    }
    ...
    for ($i = $var; $i < ($paginaAtual - 1) + $offset; $i++ ) {
      ...
      if ($i <= $numeroPaginas && $i >= 1) {
            ...
            if ($i == $var2) {
                $paginas .= " <li class='active'><a href='$urlSite/pagina-produtos/$página'>$página</a></li> ";
            }
            else {
                $paginas .= " <li><a href='$urlSite/pagina-produtos/$página'>$página</a></li> ";
            }
        }
    }
    ...
    if ($paginaAtual == $numeroPaginas) {
        ...
        $p2 = $paginaAtual + 1;
        $paginas .= "<li><a href='$urlSite/pagina-produtos/$p2'><span>&rsaquo;</span></a></li>";
        $paginas .= "<li><a href='$urlSite/pagina-produtos/$numeroPaginas'><span>&raquo;</span></a></li>";
    }

...
<html lang="pt-br">
  <head>
    ...
    <title>Meu Carrinho - ELentes</title>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            $('.quantidade').change(function(){
                ...
                var valorUnitario = $(".quantidade").val();
                
                //alert(valorUnitario);
                
                /*if ($('#quantidade').val() > 0) {
                
                    var valorUnitario = $(valorUnit).val();
        ...
        teste = function(id){
            ...
            if ($('#quantidade'+id).val() > 0) {
                ...
                var valorUnitario = $('#valorUnit'+id).val();

                var qtde = $('#quantidade'+id).val();

                var total = valorUnitario * qtde;

                total = total.toFixed(2);

                $("#valor"+id).html("R$ "+total);
                
                var aux = parseFloat(totalCarrinho);
                //var aux2 = aux +
                
                totalCarrinho += parseFloat(valorUnitario);
  ...
  <body class="fade-down">
    ...
    <div class="container m-t-3">
      <div class="row">
        ...
        <!-- Shopping Cart List -->
        <div class="col-md-12">
          <div class="title"><span>Meu Carrinho</span></div>
            ...
            <table class="table table-bordered table-cart">
              ...
              <tbody>
                        ...
                        foreach ($produto->getProduto($valor['idProduto']) as $var) {
                            ...
                            ?>
                                <tr>
                                  ...
                                  <td class="qtde"><?php echo $qtdeTotal; ?></td>
                                  <td class="unit">R$ <?php echo number_format((float)$valorUnitario, 2, '.', ''); ?></td>
                                  <td class="sub">R$ <?php echo number_format((float)$subTotal, 2, '.', '');?></td>
                                  <td class="action">
                                    <form method="POST" action="./editar-carrinho">
                                        <input type="text" name="idCarrinho" class="hidden" value="<?php echo $valor['id']; ?>"/>
                                        <button type="submit" class="text-success fafabotao" data-toggle="tooltip" data-placement="top" data-original-title="Editar"><i class="fa fa-pencil-square-o"></i></button>
                                    </form>
                                      <form method="POST" action="./controls/controlPedido.php">
                                        <input type="text" name="removerProdCarrinho" class="hidden" value="<?php echo $valor['id']; ?>"/>
                                        <button type="submit" class="text-danger fafabotao" data-toggle="tooltip" data-placement="top" data-original-title="Remover"><i class="fa fa-trash-o"></i></button>
                                    </form>
                                  </td>
                                </tr>
                            <?php                
                ...
                <tr>
                  ...
                  <td colspan="2"><b id="total">R$ <?php echo number_format((float)$total, 2, '.', ''); ?></b></td>
                </tr>
              </tbody>
            </table>
          </div>
          <nav aria-label="Shopping Cart Next Navigation">
            <ul class="pager">
              <li class="previous"><a href="produtos"><span aria-hidden="true">&larr;</span> Continuar Comprando</a></li>
              <li class="next"><a href="finalizar">Finalizar Compra<span aria-hidden="true">&rarr;</span></a></li>
            </ul>
          </nav>
        </div>
